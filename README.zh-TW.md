...
- **馬丁格爾**：逆勢加碼每筆是上一筆的固定倍數（例如 ×1.45、×2），連續錯邊時虧損以倍數放大。
- **網格**：固定份量掛在等距價位，每一份在相鄰一格平倉後再重複交易；單邊行情會一路累積持倉。
- **逆勢攤平不停損**：虧損時加碼、幾乎從不認賠，一次持續的單邊行情可能吃掉許多小賺。
- **逆勢攤平會停損**：虧損時加碼，但也會認賠出場。
- **短線交易**：多數倉位持有不到一天，跟單延遲與手續費影響較大。
- **波段交易**：持倉時間較長，跟單者可能承接既有持倉波動。
- **近期紀錄不足**：交易所只公開最近的成交紀錄（幣安約 60 天、最多約 6,000 筆訂單），這段期間內能從開倉看到平倉的完整倉位太少，所以不判斷交易風格。這不代表帶單員交易經驗少（倉位一抱好幾週的長期交易者也可能落在這裡），其他風險數據照常顯示。
