...
- **マーチンゲール**：逆行時の買い増しが毎回直前の一定倍数（例：×1.45、×2）のため、連続して逆行すると損失が倍々に膨らむ。
- **グリッド**：等間隔の価格に同じサイズの注文を置き、各注文を隣の水準で決済して繰り返す。一方向相場では在庫が積み上がる。
- **損切りなしのナンピン**：含み損で買い増し、ほぼ損切りしないため、一度の大きな一方向相場で多くの小さな利益を失いうる。
- **損切りありのナンピン**：含み損で買い増すが、損失ポジションの決済も行う。
- **短期取引**：多くのポジションを 1 日未満で保有するため、コピー遅延と手数料の影響が大きい。
- **スイング取引**：保有時間が長めで、コピー側が既存ポジションの変動を引き継ぐ可能性がある。
- **直近の記録不足**：取引所は最近の取引履歴（バイナンスは約 60 日間、最大約 6,000 件の注文）のみを公開しており、その期間内に建てから決済まで追えるポジションが少なすぎるため、取引スタイルを判断しません。トレーダーの経験が少ないという意味ではなく（数週間ポジションを保有するトレーダーも該当します）、他のリスクデータは通常どおり表示されます。
