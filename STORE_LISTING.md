...
當你打開個別跟單員頁面時，Extension 會在頁面右側顯示即時分析面板，直接從當下頁面的 Binance / OKX API 抓取最新資料。Binance 主績效會優先用歷史交易、轉帳紀錄與目前帶單資金重建，不依賴頁面目前選擇的 7D、30D、90D 或 180D 顯示區間。
...
- 本工具不是交易機器人，不會下單。
- 本工具不是投資建議，不保證收益或安全。
- 分析只根據當前頁面可取得資料，若交易所限制歷史資料或 API 改版，面板會標示資料不足或 `N/A`，不會把未知值當成 0。
