...
- **马丁格尔**：逆势加仓每笔是上一笔的固定倍数（例如 ×1.45、×2），连续错边时亏损按倍数放大。
- **网格**：固定份量挂在等距价位，每一份在相邻一格平仓后再重复交易；单边行情会一路累积持仓。
- **逆势摊平不止损**：亏损时加仓、几乎从不认赔，一次持续的单边行情可能吃掉许多小赚。
- **逆势摊平会止损**：亏损时加仓，但也会认赔出场。
- **短线交易**：多数仓位持有不到一天，跟单延迟与手续费影响较大。
- **波段交易**：持仓时间较长，跟单者可能承接既有持仓波动。
- **近期记录不足**：交易所只公开最近的成交记录（币安约 60 天、最多约 6,000 笔订单），这段期间内能从开仓看到平仓的完整仓位太少，所以不判断交易风格。这不代表带单员交易经验少（持仓好几周的长期交易者也可能落在这里），其他风险数据照常显示。
